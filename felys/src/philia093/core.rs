...
impl super::PhiLia093 {
    pub fn root(&mut self) -> Option<Root> {
        if self.__snapshot.is_some() { return None; } const RULES: super::R<Root, 1usize> = [|x| {
            let first = match x.item() {
                Some(value) => value,
            ...
            let _ = match x.EOF() {
                Some(value) => value,
                None => return x.__error("<EOF>"),
            };
            Some(Root(BufVec::new([first], more)))
        }];
        self.__peg(RULES)
    }
    pub fn item(&mut self) -> Option<Item> {
        if self.__snapshot.is_some() { return None; } const RULES: super::R<Item, 4usize> = [|x| {
            let _ = x.GROUP()?;
            let ident = match x.IDENT() {
                Some(value) => value,
                None => return x.__error("<IDENT>"),
            };
            let _ = match x.LPAR() {
                Some(value) => value,
                None => return x.__error("<LPAR>"),
            };
            let params = match x.params() {
                Some(value) => value,
                None => return x.__error("<params>"),
            };
            let _ = match x.RPAR() {
                Some(value) => value,
            ...
            let _ = match x.LBRA() {
                Some(value) => value,
                None => return x.__error("<LBRA>"),
            };
            let first = match x.implementation() {
                Some(value) => value,
                None => return x.__error("<implementation>"),
            };
            let more = {
                let mut body = Vec::new();
                while let Some(data) = x.implementation() { body.push(data) }
                body
            };
            let _ = match x.RBRA() {
                Some(value) => value,
            ...
            Some(Item::Impl(ident, BufVec::new([first], more)))
        }, |x| {
            let _ = x.FN()?;
            let _ = x.MAIN()?;
            let _ = match x.LPAR() {
            ...
            let block = match x.block() {
                Some(value) => value,
                None => return x.__error("<block>"),
            };
            Some(Item::Main(ident, block))
        }, |x| {
            let _ = x.FN()?;
            let ident = match x.IDENT() {
                Some(value) => value,
    ...
    pub fn implementation(&mut self) -> Option<Impl> {
        if self.__snapshot.is_some() { return None; } const RULES: super::R<Impl, 2usize> = [|x| {
            let _ = x.FN()?;
            let ident = match x.IDENT() {
                Some(value) => value,
                None => return x.__error("<IDENT>"),
            ...
            Some(Impl::Method(ident, more, block))
        }, |x| {
            let _ = x.FN()?;
            let ident = match x.IDENT() {
                Some(value) => value,
            ...
            Some(Impl::Associated(ident, params, block))
        }];
        self.__peg(RULES)
    }
    pub fn pat(&mut self) -> Option<Pat> {
        if self.__snapshot.is_some() { return None; } const RULES: super::R<Pat, 3usize> = [|x| {
            let _ = x.UNDERSCORE()?;
            Some(Pat::Any)
        }, |x| {
            let ident = x.IDENT()?;
            ...
            let _ = x.COMMA();
            let _ = x.RPAR()?;
            Some(Pat::Tuple(BufVec::new([first, second], more)))
        }];
        let start = self.__stream.cursor;
        if let Some((end, cache)) = self.__memo.pat.get(&start) {
        ...
        self.__memo.pat.insert(start, (end, cache));
        result
    }
    pub fn block(&mut self) -> Option<Block> {
        if self.__snapshot.is_some() { return None; } const RULES: super::R<Block, 1usize> = [|x| {
            let _ = x.LBRA()?;
            let stmts = {
                let mut body = Vec::new();
                while let Some(data) = x.stmt() { body.push(data) }
                body
            };
            let _ = match x.RBRA() {
                Some(value) => value,
    ...
    pub fn stmt(&mut self) -> Option<Stmt> {
        if self.__snapshot.is_some() { return None; } const RULES: super::R<Stmt, 4usize> = [|x| {
    ...
    pub fn assignment(&mut self) -> Option<Stmt> {
        if self.__snapshot.is_some() { return None; } const RULES: super::R<Stmt, 6usize> = [|x| {
            let pat = x.pat()?;
            let _ = x.EQ()?;
            ...
            Some(Stmt::Assign(pat, AssOp::Eq, expr))
        }, |x| {
            let pat = x.pat()?;
            let _ = x.ADDEQ()?;
            let expr = match x.expr() {
            ...
            Some(Stmt::Assign(pat, AssOp::AddEq, expr))
        }, |x| {
            let pat = x.pat()?;
            let _ = x.SUBEQ()?;
            let expr = match x.expr() {
            ...
            Some(Stmt::Assign(pat, AssOp::SubEq, expr))
        }, |x| {
            let pat = x.pat()?;
            let _ = x.MULEQ()?;
            let expr = match x.expr() {
            ...
            Some(Stmt::Assign(pat, AssOp::MulEq, expr))
        }, |x| {
            let pat = x.pat()?;
            let _ = x.DIVEQ()?;
            let expr = match x.expr() {
            ...
            Some(Stmt::Assign(pat, AssOp::DivEq, expr))
        }, |x| {
            let pat = x.pat()?;
            let _ = x.MODEQ()?;
            let expr = match x.expr() {
            ...
            Some(Stmt::Assign(pat, AssOp::ModEq, expr))
        }];
        self.__peg(RULES)
    }
    pub fn expr(&mut self) -> Option<Expr> {
        if self.__snapshot.is_some() { return None; } const RULES: super::R<Expr, 6usize> = [|x| {
            ...
            let pat = match x.pat() {
                Some(value) => value,
                None => return x.__error("<pat>"),
            };
            let _ = match x.IN() {
                Some(value) => value,
                None => return x.__error("<IN>"),
            };
            let expr = match x.expr() {
                Some(value) => value,
                None => return x.__error("<expr>"),
        ...
        let start = self.__stream.cursor;
        if let Some((end, cache)) = self.__memo.expr.get(&start) {
            self.__stream.cursor = end.to_owned();
            return cache.clone();
        }
        let result = self.__peg(RULES);
        let end = self.__stream.cursor;
        let cache = result.clone();
        self.__memo.expr.insert(start, (end, cache));
        result
    }
    pub fn disjunction(&mut self) -> Option<Expr> {
        if self.__snapshot.is_some() { return None; } const RULES: super::R<Expr, 2usize> = [|x| {
            let lhs = x.disjunction()?;
            let _ = x.OR()?;
            let rhs = match x.conjunction() {
                Some(value) => value,
                None => return x.__error("<conjunction>"),
            };
        ...
        let start = self.__stream.cursor;
        if let Some((end, cache)) = self.__memo.disjunction.get(&start) {
            self.__stream.cursor = end.to_owned();
            return cache.clone();
        }
        let mut result = None;
        ...
        loop {
            let cache = result.clone();
            self.__memo.disjunction.insert(start, (end, cache));
    ...
    pub fn conjunction(&mut self) -> Option<Expr> {
        if self.__snapshot.is_some() { return None; } const RULES: super::R<Expr, 2usize> = [|x| {
            ...
            let rhs = match x.inversion() {
                Some(value) => value,
                None => return x.__error("<inversion>"),
            };
        ...
        let start = self.__stream.cursor;
        if let Some((end, cache)) = self.__memo.conjunction.get(&start) {
            self.__stream.cursor = end.to_owned();
            return cache.clone();
        }
        let mut result = None;
        ...
        loop {
            let cache = result.clone();
            self.__memo.conjunction.insert(start, (end, cache));
            let temp = self.__peg(RULES);
            if end < self.__stream.cursor {
                result = temp;
                end = self.__stream.cursor;
    ...
    pub fn equality(&mut self) -> Option<Expr> {
        if self.__snapshot.is_some() { return None; } const RULES: super::R<Expr, 3usize> = [|x| {
            let lhs = x.equality()?;
            let _ = x.EQ2()?;
            let rhs = match x.comparison() {
                Some(value) => value,
                None => return x.__error("<comparison>"),
            };
        ...
        let start = self.__stream.cursor;
        if let Some((end, cache)) = self.__memo.equality.get(&start) {
            self.__stream.cursor = end.to_owned();
            return cache.clone();
        }
        let mut result = None;
        ...
        loop {
            let cache = result.clone();
            self.__memo.equality.insert(start, (end, cache));
            let temp = self.__peg(RULES);
            if end < self.__stream.cursor {
                result = temp;
                end = self.__stream.cursor;
    ...
    pub fn comparison(&mut self) -> Option<Expr> {
        if self.__snapshot.is_some() { return None; } const RULES: super::R<Expr, 5usize> = [|x| {
            let lhs = x.comparison()?;
            let _ = x.GE()?;
            let rhs = match x.term() {
                Some(value) => value,
                None => return x.__error("<term>"),
            };
        ...
        let start = self.__stream.cursor;
        if let Some((end, cache)) = self.__memo.comparison.get(&start) {
            self.__stream.cursor = end.to_owned();
            return cache.clone();
        }
        let mut result = None;
        ...
        loop {
            let cache = result.clone();
            self.__memo.comparison.insert(start, (end, cache));
            let temp = self.__peg(RULES);
            if end < self.__stream.cursor {
                result = temp;
                end = self.__stream.cursor;
    ...
    pub fn term(&mut self) -> Option<Expr> {
        ...
        let start = self.__stream.cursor;
        if let Some((end, cache)) = self.__memo.term.get(&start) {
            self.__stream.cursor = end.to_owned();
            return cache.clone();
        }
        let mut result = None;
        ...
        loop {
            let cache = result.clone();
            self.__memo.term.insert(start, (end, cache));
            let temp = self.__peg(RULES);
            if end < self.__stream.cursor {
                result = temp;
                end = self.__stream.cursor;
    ...
    pub fn factor(&mut self) -> Option<Expr> {
        if self.__snapshot.is_some() { return None; } const RULES: super::R<Expr, 4usize> = [|x| {
            let lhs = x.factor()?;
            let _ = x.ASTERISK()?;
            let rhs = match x.dot() {
                Some(value) => value,
        ...
        let start = self.__stream.cursor;
        if let Some((end, cache)) = self.__memo.factor.get(&start) {
            self.__stream.cursor = end.to_owned();
            return cache.clone();
        }
        let mut result = None;
    ...
    pub fn call(&mut self) -> Option<Expr> {
        if self.__snapshot.is_some() { return None; } const RULES: super::R<Expr, 5usize> = [|x| {
            ...
            Some(Expr::Method(call.into(), ident, args))
        }, |x| {
            let call = x.call()?;
            let _ = x.DOT()?;
            let ident = match x.IDENT() {
            ...
            Some(Expr::Field(call.into(), ident))
        }, |x| {
            let call = x.call()?;
            let _ = x.LPAR()?;
            let args = x.args();
            ...
            let _ = match x.RSQB() {
                Some(value) => value,
                None => return x.__error("<RSQB>"),
            };
            Some(Expr::Index(call.into(), index.into()))
        }, |x| {
            let primary = x.primary()?;
            Some((primary))
        }];
        let start = self.__stream.cursor;
        if let Some((end, cache)) = self.__memo.call.get(&start) {
            self.__stream.cursor = end.to_owned();
            return cache.clone();
        }
        let mut result = None;
        ...
        loop {
            let cache = result.clone();
            self.__memo.call.insert(start, (end, cache));
            let temp = self.__peg(RULES);
            if end < self.__stream.cursor {
                result = temp;
                end = self.__stream.cursor;
    ...
    pub fn args(&mut self) -> Option<BufVec<Expr, 1>> {
        if self.__snapshot.is_some() { return None; } const RULES: super::R<BufVec<Expr, 1>, 1usize> = [|x| {
            let first = x.expr()?;
            let more = {
                let mut body = Vec::new();
                while let Some(data) = x.__peg([|x| {
    ...
    pub fn primary(&mut self) -> Option<Expr> {
        if self.__snapshot.is_some() { return None; } const RULES: super::R<Expr, 8usize> = [|x| {
            let lit = x.lit()?;
            Some(Expr::Lit(lit))
        }, |x| {
            let path = x.path()?;
            ...
            Some(Expr::Tuple(BufVec::new([first, second], more)))
        }, |x| {
            let _ = x.LSQB()?;
            let args = x.args();
            let _ = match x.RSQB() {
    ...
    pub fn params(&mut self) -> Option<BufVec<usize, 1>> {
    ...
    pub fn path(&mut self) -> Option<Path> {
        if self.__snapshot.is_some() { return None; } const RULES: super::R<Path, 1usize> = [|x| {
            let first = x.IDENT()?;
            let more = {
                let mut body = Vec::new();
                while let Some(data) = x.__peg([|x| {
    ...
    pub fn lit(&mut self) -> Option<Lit> {
        if self.__snapshot.is_some() { return None; } const RULES: super::R<Lit, 4usize> = [|x| {
            let float = x.FLOAT()?;
    ...
    pub fn STR(&mut self) -> Option<Vec<Chunk>> {
        if self.__snapshot.is_some() { return None; } const RULES: super::R<Vec<Chunk>, 1usize> = [|x| {
            let _ = {
                let mut body = Vec::new();
                while let Some(data) = x.T() { body.push(data) }
                body
            ...
            let chunks = {
                let mut body = Vec::new();
                while let Some(data) = x.CHUNK() { body.push(data) }
                body
            };
    ...
    pub fn BOOL(&mut self) -> Option<Bool> {
        if self.__snapshot.is_some() { return None; } const RULES: super::R<Bool, 2usize> = [|x| {
            let _ = {
                let mut body = Vec::new();
                while let Some(data) = x.T() { body.push(data) }
                body
    ...
    pub fn MAIN(&mut self) -> Option<()> {
        if self.__snapshot.is_some() { return None; } const RULES: super::R<(), 1usize> = [|x| {
            ...
            let _ = x.__expect("main")?;
            let _ = x.__lookahead(|x| x.TAIL(), false)?;
            Some(())
        }];
        self.__peg(RULES)
    }
    pub fn GROUP(&mut self) -> Option<()> {
        if self.__snapshot.is_some() { return None; } const RULES: super::R<(), 1usize> = [|x| {
            let _ = {
                let mut body = Vec::new();
                while let Some(data) = x.T() { body.push(data) }
            ...
            let _ = x.__expect("group")?;
            let _ = x.__lookahead(|x| x.TAIL(), false)?;
            Some(())
        }];
        self.__peg(RULES)
    }
    pub fn IMPL(&mut self) -> Option<()> {
        if self.__snapshot.is_some() { return None; } const RULES: super::R<(), 1usize> = [|x| {
            let _ = {
                let mut body = Vec::new();
                while let Some(data) = x.T() { body.push(data) }
            ...
            let _ = x.__expect("impl")?;
            let _ = x.__lookahead(|x| x.TAIL(), false)?;
            Some(())
        }];
        self.__peg(RULES)
    }
    pub fn IF(&mut self) -> Option<()> {
        if self.__snapshot.is_some() { return None; } const RULES: super::R<(), 1usize> = [|x| {
            let _ = {
                let mut body = Vec::new();
                while let Some(data) = x.T() { body.push(data) }
    ...
    pub fn CONTINUE(&mut self) -> Option<()> {
        if self.__snapshot.is_some() { return None; } const RULES: super::R<(), 1usize> = [|x| {
            let _ = {
                let mut body = Vec::new();
                while let Some(data) = x.T() { body.push(data) }
    ...
    pub fn SELF(&mut self) -> Option<()> {
        if self.__snapshot.is_some() { return None; } const RULES: super::R<(), 1usize> = [|x| {
            ...
            let _ = x.__expect("self")?;
            let _ = x.__lookahead(|x| x.TAIL(), false)?;
            Some(())
        }];
        self.__peg(RULES)
    }
    pub fn OR(&mut self) -> Option<()> {
        if self.__snapshot.is_some() { return None; } const RULES: super::R<(), 1usize> = [|x| {
            let _ = {
                let mut body = Vec::new();
                while let Some(data) = x.T() { body.push(data) }
    ...
    pub fn ASTERISK(&mut self) -> Option<()> {
        if self.__snapshot.is_some() { return None; } const RULES: super::R<(), 1usize> = [|x| {
            let _ = {
                let mut body = Vec::new();
                while let Some(data) = x.T() { body.push(data) }
    ...
    pub fn PERCENT(&mut self) -> Option<()> {
        if self.__snapshot.is_some() { return None; } const RULES: super::R<(), 1usize> = [|x| {
            let _ = {
                let mut body = Vec::new();
                while let Some(data) = x.T() { body.push(data) }
    ...
    pub fn UNDERSCORE(&mut self) -> Option<()> {
        if self.__snapshot.is_some() { return None; } const RULES: super::R<(), 1usize> = [|x| {
            let _ = {
                let mut body = Vec::new();
                while let Some(data) = x.T() { body.push(data) }
    ...
    pub fn CHUNK(&mut self) -> Option<Chunk> {
        if self.__snapshot.is_some() { return None; } const RULES: super::R<Chunk, 3usize> = [|x| {
            let slice = x.SLICE()?;
    ...
    pub fn FXX(&mut self) -> Option<usize> {
        if self.__snapshot.is_some() { return None; } fn transition(s: usize, c: char) -> Option<usize> {
            let s = match (s, c as usize) {
                (0usize, 48usize..=48usize) => 1usize,
                (0usize, 49usize..=57usize) => 2usize,
    ...
    pub fn HEX(&mut self) -> Option<usize> {
        if self.__snapshot.is_some() { return None; } fn transition(s: usize, c: char) -> Option<usize> {
            let s = match (s, c as usize) {
                (0usize, 48usize..=57usize) => 1usize,
                (0usize, 65usize..=70usize) => 1usize,
                (0usize, 97usize..=102usize) => 1usize,
                (1usize, 48usize..=57usize) => 1usize,
                (1usize, 65usize..=70usize) => 1usize,
                (1usize, 97usize..=102usize) => 1usize,
                _ => return None,
            };
            Some(s)
        }
