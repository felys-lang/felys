...
impl super::Packrat {
        ...
        const RULES: super::Rules<Expr, 2usize> = [
            |x| {
                ...
                let elements = {
                    ...
                    while let Some(res) = x.attempt(|x| {
                        x.COMMA()?;
                        x.expr()
                    }) {
                        body.push(res)
    ...
    pub fn disjunction(&mut self) -> Option<Expr> {
        ...
        const RULES: super::Rules<Expr, 2usize> = [
            |x| {
                ...
                let rhs = match x.conjunction() {
                    Some(value) => value,
                    None => return x.error("conjunction"),
                };
                Some(Expr::Binary(lhs.into(), BinOp::Or, rhs.into()))
            },
            |x| {
                let conjunction = match x.conjunction() {
                    Some(value) => value,
                    None => return x.error("conjunction"),
                };
                Some(conjunction)
        ...
        loop {
            ...
            let temp = self.sequential(RULES);
        ...
        const RULES: super::Rules<Expr, 3usize> = [
            |x| {
                ...
                let rhs = match x.comparison() {
                    Some(value) => value,
                    None => return x.error("comparison"),
                };
