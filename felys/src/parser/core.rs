...
impl super::Packrat {
    ...
    pub fn pat(&mut self) -> Option<Pat> {
        ...
        const RULES: super::Rules<Pat, 4usize> = [
            |x| {
                let _ = x.UNDERSCORE()?;
                Some(Pat::Any)
            },
        ...
        if let Some((end, cache)) = self.memo.pat.get(&(start, strict)) {
        ...
        self.memo.pat.insert((start, strict), (end, cache));
        result
    }
    ...
    pub fn call(&mut self) -> Option<Expr> {
        ...
        let start = self.stream.cursor;
        let strict = self.stream.strict;
        if let Some((end, cache)) = self.memo.call.get(&(start, strict)) {
            self.stream.cursor = end.to_owned();
            return cache.clone();
        }
        let mut result = None;
        ...
        loop {
            let cache = result.clone();
            self.memo.call.insert((start, strict), (end, cache));
            let temp = self.__peg(RULES);
            if end < self.stream.cursor {
                result = temp;
                end = self.stream.cursor;
    ...
    pub fn args(&mut self) -> Option<BufVec<Expr, 1>> {
        ...
        const RULES: super::Rules<BufVec<Expr, 1>, 1usize> = [|x| {
            let first = x.expr()?;
            let more = {
                let mut body = Vec::new();
                while let Some(data) = x.__peg([|x| {
    ...
    pub fn primary(&mut self) -> Option<Expr> {
        ...
        const RULES: super::Rules<Expr, 7usize> = [
            |x| {
                let lit = x.lit()?;
                Some(Expr::Lit(lit))
            },
            ...
            |x| {
                ...
                Some(Expr::Closure(params, expr.into()))
            },
        ];
        self.__peg(RULES)
    }
    pub fn row(&mut self) -> Option<BufVec<Float, 1>> {
        if self.snapshot.is_some() {
            return None;
        }
        const RULES: super::Rules<BufVec<Float, 1>, 1usize> = [|x| {
            let first = x.FLOAT()?;
            let more = {
                let mut body = Vec::new();
                while let Some(data) = x.__peg([|x| {
    ...
    pub fn params(&mut self) -> Option<BufVec<Ident, 1>> {
        if self.snapshot.is_some() {
            return None;
        }
        const RULES: super::Rules<BufVec<Ident, 1>, 1usize> = [|x| {
            let first = x.ident()?;
            let more = {
                let mut body = Vec::new();
                while let Some(data) = x.__peg([|x| {
    ...
    pub fn UNDERSCORE(&mut self) -> Option<()> {
        ...
        const RULES: super::Rules<(), 1usize> = [|x| {
            let _ = x.__expect("_")?;
            let _ = x.__lookahead(|x| x.TAIL(), false)?;
            Some(())
        }];
    ...
    pub fn IDENT(&mut self) -> Option<usize> {
        ...
        let start = self.stream.cursor;
        let strict = self.stream.strict;
        if let Some(&(end, cache)) = self.memo.IDENT.get(&(start, strict)) {
            self.stream.cursor = end;
            return cache;
        }
        self.stream.trim();
        ...
        let end = self.stream.cursor;
        self.memo.IDENT.insert((start, strict), (end, result));
        result
    }
