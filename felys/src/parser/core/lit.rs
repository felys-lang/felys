...
impl Literal for Parser<CR> {
    ...
    fn int(&mut self) -> Option<Int> {
        if let Some(res) = self.alter(|x| {
            ...
            let first = x.e("incomplete hexadecimal").scan(|c| c.is_ascii_hexdigit())?;
        ...
        if let Some(res) = self.alter(|x| {
            x.expect("0o")?;
            x.stream.strict = true;
            let first = x.e("incomplete octal").scan(|c| matches!(c, '0'..='7'))?;
            let mut body = String::from(first);
        ...
        if let Some(res) = self.alter(|x| {
            x.expect("0b")?;
            x.stream.strict = true;
            let first = x.e("incomplete binary").scan(|c| matches!(c, '0'|'1'))?;
            let mut body = String::from(first);
        ...
        if let Some(res) = self.alter(|x| {
            x.expect("0")?;
            x.stream.strict = true;
            x.e("decimal start with zero").lookahead(|c| !c.is_ascii_digit())?;
    ...
    fn float(&mut self) -> Option<Float> {
        if let Some(res) = self.alter(|x| {
            ...
            x.stream.strict = true;
            x.expect(".")?;
            let first = x.e("incomplete float").scan(|c| c.is_ascii_digit())?;
    ...
    fn str(&mut self) -> Option<Str> {
        if let Some(res) = self.alter(|x| {
            ...
            x.e("string not unclosed").expect("\"")?;
            let id = x.intern.id(body);
            Some(id.into())
        }) {
            return res;
