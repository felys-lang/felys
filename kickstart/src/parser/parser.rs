...
impl super::Packrat {
    ...
    pub fn callable(&mut self) -> Option<Callable> {
        ...
        const RULES: super::Rules<Callable, 3usize> = [
            |x| {
                ...
                Some(Callable::Rule(decorator, name, ty, rule))
            },
            |x| {
                let decorator = x.decorator();
                let name = x.NAME()?;
                ...
                Some(Callable::Regex(decorator, name, regex))
            },
            |x| {
                let decorator = x.decorator()?;
                let _ = match x.__expect("{") {
                ...
                Some(Callable::Shared(decorator, shared))
            },
        ];
        self.__rule(RULES)
    }
    pub fn decorator(&mut self) -> Option<Decorator> {
        if self.snapshot.is_some() {
            return None;
        }
        const RULES: super::Rules<Decorator, 1usize> = [|x| {
            let _ = x.__expect("@")?;
            let _ = match x.__expect("(") {
                Some(value) => value,
                None => return x.__error("'('"),
            };
            let first = match x.tag() {
                Some(value) => value,
    ...
    pub fn tag(&mut self) -> Option<Tag> {
        ...
        const RULES: super::Rules<Tag, 5usize> = [
            |x| {
            ...
            |x| {
                let _ = x.__expect("token")?;
                Some(Tag::Token)
            },
            |x| {
                let _ = x.__expect("intern")?;
                Some(Tag::Intern)
            },
            |x| {
                let _ = x.__expect("ws")?;
    ...
    pub fn rule(&mut self) -> Option<Rule> {
        ...
        const RULES: super::Rules<Rule, 1usize> = [|x| {
            let _ = x.__expect("/");
    ...
    pub fn alter(&mut self) -> Option<Alter> {
        ...
        const RULES: super::Rules<Alter, 1usize> = [|x| {
            let assignments = {
    ...
    pub fn assignment(&mut self) -> Option<Assignment> {
        ...
        const RULES: super::Rules<Assignment, 4usize> = [
            |x| {
                let name = x.NAME()?;
                let _ = x.__expect("=")?;
    ...
    pub fn lookahead(&mut self) -> Option<Lookahead> {
        ...
        const RULES: super::Rules<Lookahead, 2usize> = [
            |x| {
                let _ = x.__expect("&")?;
    ...
    pub fn item(&mut self) -> Option<Item> {
        ...
        const RULES: super::Rules<Item, 4usize> = [
            |x| {
                let atom = x.atom()?;
                let _ = x.__expect("?")?;
            ...
            |x| {
                let eager = x.__expect("#");
                let atom = x.atom()?;
                let _ = x.__expect("+")?;
                Some(Item::OnceOrMore(eager.is_some(), atom))
            },
            |x| {
                let eager = x.__expect("#");
                let atom = x.atom()?;
                Some(Item::Name(eager.is_some(), atom))
            },
        ];
        self.__rule(RULES)
    }
    pub fn atom(&mut self) -> Option<Atom> {
        ...
        const RULES: super::Rules<Atom, 3usize> = [
            |x| {
                let decorator = x.decorator();
                let _ = x.__expect("(")?;
                let rule = match x.rule() {
                ...
                Some(Atom::Nested(decorator, rule))
            },
            |x| {
                let string = x.string()?;
    ...
    pub fn union(&mut self) -> Option<Regex> {
        ...
        const RULES: super::Rules<Regex, 2usize> = [
            |x| {
                let lhs = x.union()?;
                let _ = x.__expect("|")?;
        ...
        if let Some((end, cache)) = self.memo.union.get(&(start, strict)) {
        ...
        loop {
            ...
            self.memo.union.insert((start, strict), (end, cache));
    ...
    pub fn concat(&mut self) -> Option<Regex> {
        ...
        let start = self.stream.cursor;
        let strict = self.stream.strict;
        if let Some((end, cache)) = self.memo.concat.get(&(start, strict)) {
            self.stream.cursor = end.to_owned();
            return cache.clone();
        }
        let mut result = None;
        ...
        loop {
            let cache = result.clone();
            self.memo.concat.insert((start, strict), (end, cache));
            let temp = self.__rule(RULES);
            if end < self.stream.cursor {
                result = temp;
                end = self.stream.cursor;
    ...
    pub fn repeat(&mut self) -> Option<Regex> {
        ...
        const RULES: super::Rules<Regex, 3usize> = [
            |x| {
                let inner = x.repeat()?;
                let _ = x.__expect("+")?;
        ...
        let start = self.stream.cursor;
        let strict = self.stream.strict;
        if let Some((end, cache)) = self.memo.repeat.get(&(start, strict)) {
            self.stream.cursor = end.to_owned();
            return cache.clone();
        }
        let mut result = None;
        ...
        loop {
            let cache = result.clone();
            self.memo.repeat.insert((start, strict), (end, cache));
            let temp = self.__rule(RULES);
            if end < self.stream.cursor {
                result = temp;
                end = self.stream.cursor;
    ...
    pub fn primary(&mut self) -> Option<Primary> {
        ...
        const RULES: super::Rules<Primary, 4usize> = [
            |x| {
                let _ = x.__expect("(")?;
                let regex = match x.union() {
                    Some(value) => value,
    ...
    pub fn set(&mut self) -> Option<Primary> {
        ...
        const RULES: super::Rules<Primary, 2usize> = [
            |x| {
                let _ = x.__expect("[")?;
                let _ = x.__expect("^")?;
                let set = {
            ...
            |x| {
                ...
                Some(Primary::Include(set))
            },
        ];
        self.__token(RULES)
    }
    pub fn string(&mut self) -> Option<Vec<usize>> {
        if self.snapshot.is_some() {
            return None;
        }
        const RULES: super::Rules<Vec<usize>, 1usize> = [|x| {
            let _ = x.__expect("'")?;
            let string = {
                let first = match x.LCH() {
                    Some(value) => value,
                    None => return x.__error("<LCH>"),
                };
                let mut body = Vec::from([first]);
                while let Some(data) = x.LCH() {
                    body.push(data)
                }
                body
            };
    ...
    pub fn range(&mut self) -> Option<(usize, usize)> {
        ...
        const RULES: super::Rules<(usize, usize), 2usize> = [
            |x| {
                let start = x.SCH()?;
                let _ = x.__expect("-")?;
                let end = match x.SCH() {
                    Some(value) => value,
                    None => return x.__error("<SCH>"),
                };
                Some((start, end))
            },
            |x| {
    ...
    pub fn LCH(&mut self) -> Option<usize> {
        if self.snapshot.is_some() {
            return None;
        }
        fn transition(s: usize, c: char) -> Option<usize> {
            let s = match (s, c as usize) {
                ...
                (2usize, 39usize..=39usize) => 1usize,
                (2usize, 91usize..=91usize) => 1usize,
                (2usize, 92usize..=92usize) => 1usize,
                (2usize, 93usize..=93usize) => 1usize,
                (2usize, 110usize..=110usize) => 1usize,
                (2usize, 114usize..=114usize) => 1usize,
                (2usize, 116usize..=116usize) => 1usize,
                (2usize, 117usize..=117usize) => 3usize,
                (3usize, 123usize..=123usize) => 4usize,
                (4usize, 48usize..=57usize) => 4usize,
                (4usize, 97usize..=102usize) => 4usize,
                (4usize, 125usize..=125usize) => 1usize,
                _ => return None,
            };
            Some(s)
        }
    ...
    pub fn SCH(&mut self) -> Option<usize> {
        if self.snapshot.is_some() {
            return None;
        }
        fn transition(s: usize, c: char) -> Option<usize> {
            let s = match (s, c as usize) {
                ...
                (0usize, 92usize..=92usize) => 2usize,
                (0usize, 94usize..=18446744073709551615usize) => 1usize,
                (2usize, 39usize..=39usize) => 1usize,
                (2usize, 91usize..=91usize) => 1usize,
                (2usize, 92usize..=92usize) => 1usize,
                (2usize, 93usize..=93usize) => 1usize,
    ...
    pub fn UNICODE(&mut self) -> Option<()> {
        if self.snapshot.is_some() {
            return None;
        }
        fn transition(s: usize, c: char) -> Option<usize> {
            let s = match (s, c as usize) {
                ...
                (2usize, 123usize..=123usize) => 3usize,
                (3usize, 48usize..=57usize) => 3usize,
                (3usize, 97usize..=102usize) => 3usize,
                (3usize, 125usize..=125usize) => 4usize,
                _ => return None,
            };
            Some(s)
        }
    ...
    pub fn ESCAPE(&mut self) -> Option<()> {
        ...
        fn transition(s: usize, c: char) -> Option<usize> {
            let s = match (s, c as usize) {
                (0usize, 92usize..=92usize) => 1usize,
                (1usize, 39usize..=39usize) => 2usize,
                (1usize, 91usize..=91usize) => 2usize,
                (1usize, 92usize..=92usize) => 2usize,
                (1usize, 93usize..=93usize) => 2usize,
                (1usize, 110usize..=110usize) => 2usize,
                (1usize, 114usize..=114usize) => 2usize,
                (1usize, 116usize..=116usize) => 2usize,
                _ => return None,
            };
            Some(s)
        }
